...
def load_model(model_key: str):
    """
    Load the model for a specific key from MODEL_CONFIG.
    """
    ...
    if model_type in ["bert", "roberta"]:
        # Choose the appropriate tokenizer and model class
        ...
        def predict_fn(text: str):
            """
            Generate predictions for the given text using the loaded model.
            """
        ...
        # T5 uses T5Tokenizer and T5ForConditionalGeneration
        tokenizer = T5Tokenizer.from_pretrained(HF_REPO_ID, subfolder=subfolder)  # no use_fast for T5
        model = T5ForConditionalGeneration.from_pretrained(HF_REPO_ID, subfolder=subfolder).to(device).eval()

        def predict_fn(text: str):
            """
            Generate predictions for the given text using T5.
            """
...
def get_model_keys():
    """
    Return a list of available model keys from the configuration.
    """
